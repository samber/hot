...
func TestComposeInternalCache(t *testing.T) {
	...
	is.Panics(func() {
		_ = composeInternalCache[string, int](true, ARC, 0, 0, nil)
	})
	...
	is.Panics(func() {
		_ = composeInternalCache[string, int](false, ARC, 0, 0, nil)
	})
...
func TestHotCacheConfig(t *testing.T) {
	...
	is.EqualValues(HotCacheConfig[string, int]{LRU, 42, false, 0, 0, 0, 0, 0, 0, nil, false, false, nil, nil, nil, nil, nil}, opts)

	opts = opts.WithMissingSharedCache()
	is.EqualValues(HotCacheConfig[string, int]{LRU, 42, true, 0, 0, 0, 0, 0, 0, nil, false, false, nil, nil, nil, nil, nil}, opts)

	opts = NewHotCache[string, int](LRU, 42).WithMissingCache(LFU, 21)
	is.EqualValues(HotCacheConfig[string, int]{LRU, 42, false, LFU, 21, 0, 0, 0, 0, nil, false, false, nil, nil, nil, nil, nil}, opts)

	is.Panics(func() {
		opts = opts.WithTTL(-42 * time.Second)
	})
	opts = opts.WithTTL(42 * time.Second)
	is.EqualValues(HotCacheConfig[string, int]{LRU, 42, false, LFU, 21, 42 * time.Second, 0, 0, 0, nil, false, false, nil, nil, nil, nil, nil}, opts)

	is.Panics(func() {
		opts = opts.WithRevalidation(-21 * time.Second)
	})
	// opts = opts.WithRevalidation(21*time.Second, loader1, loader2)
	// is.EqualValues(HotCacheConfig[string, int]{LRU, 42, false, LFU, 21, 42 * time.Second, 21 * time.Second, 0,0, nil, false, false, nil, nil, loaders, nil, nil}, opts)
	...
	opts = opts.WithJitter(0.1)
	is.EqualValues(HotCacheConfig[string, int]{LRU, 42, false, LFU, 21, 42 * time.Second, 0, 0.1, 0, nil, false, false, nil, nil, nil, nil, nil}, opts)

	// opts = opts.WithWarmUp(warmUp)
	// is.EqualValues(HotCacheConfig[string, int]{LRU, 42, false, LFU, 21, 42 * time.Second, 0, 0.1, 0, nil,false, false, warmUp, nil, nil, nil, nil}, opts)

	opts = opts.WithoutLocking()
	is.EqualValues(HotCacheConfig[string, int]{LRU, 42, false, LFU, 21, 42 * time.Second, 0, 0.1, 0, nil, true, false, nil, nil, nil, nil, nil}, opts)

	opts = opts.WithJanitor()
	is.EqualValues(HotCacheConfig[string, int]{LRU, 42, false, LFU, 21, 42 * time.Second, 0, 0.1, 0, nil, true, true, nil, nil, nil, nil, nil}, opts)

	// opts = opts.WithCopyOnRead(twice)
	// is.EqualValues(HotCacheConfig[string, int]{LRU, 42, false, LFU, 21, 42 * time.Second, 0, 0.1,0, nil, true, true, nil, nil, nil, twice, nil}, opts)

	// opts = opts.WithCopyOnRead(twice)
	// is.EqualValues(HotCacheConfig[string, int]{LRU, 42, false, LFU, 21, 42 * time.Second, 0, 0.1, 0, nil,true, true, nil, nil, nil, twice, twice}, opts)
