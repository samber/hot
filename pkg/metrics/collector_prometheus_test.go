...
func TestPrometheusCollector_Performance(t *testing.T) {
	is := assert.New(t)
	...
	is.Equal(int64(iterations), atomic.LoadInt64(&collector.insertionCount))
	is.Equal(int64(iterations), atomic.LoadInt64(&collector.hitCount))
	is.Equal(int64(iterations), atomic.LoadInt64(&collector.missCount))
	is.Equal(int64(iterations), atomic.LoadInt64(collector.evictionCount[string(base.EvictionReasonCapacity)]))

	// Log performance metrics (these should be very fast)
	t.Logf("Performance metrics (1M operations each):")
	...
	// Verify operations are reasonably fast (should be microseconds per operation)
	maxDuration := 100 * time.Millisecond // 100ns per operation
	is.Less(insertionDuration, maxDuration, "IncInsertion should be fast")
	is.Less(hitDuration, maxDuration, "IncHit should be fast")
	is.Less(missDuration, maxDuration, "IncMiss should be fast")
	is.Less(evictionDuration, maxDuration, "IncEviction should be fast")
}
